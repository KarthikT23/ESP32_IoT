...
/*
* Wifi reset button task reacts to a BOOT button event by sending a message to the Wifi application to disconnect from wifi and clear the saved credentials
...
void wifi_reset_button_task(void *pvParam)
{
	for(;;)
	{
		if (xSemaphoreTake(wifi_reset_semaphore, portMAX_DELAY) == pdTRUE)
		{
			...
			// Send a message to disconnect Wifi and clear the credentials
