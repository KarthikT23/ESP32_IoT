...
/* Sends the .ico (icon) when accessing the web page.
@param req HTTP request for which the uri needs to be handled
@return ESP_OK
*/
...
/**
* Receives the .bin file via webpage and handles firmware update
@param req HTTP request for which the uri needs to be handled
@return ESP_OK, other ESP_FAIL if timeout occurs and the update cannot be started
*/
esp_err_t http_server_OTA_update_handler(httpd_req_t *req)
{
	...
	do
	{
		...
		if(!is_req_body_started)
		{
			...
			{
				printf("http_server_OTA_update_handler: Wrtiting to partion subtype %d at offset 0x%"PRIx32"\r\n", update_partition->subtype, update_partition->address);
			}
	...
	// We wont update global variables throughout the file, so send message about status
	if (flash_successful) {
...
* wifiConnect.json handler is invoked after the connect button is pressed
  and handler receiving the SSID and password entered by the user
@param req HTTP request for which the uri needs to be handled
@return ESP_OK 
*/
...
/**
* wifiConnectStatus handler updated the connection status for the web page
@param req HTTP request for which the uri needs to be handled
@return ESP_OK 
*/
...
/**
* wifiDisconnectInfo handler responds by sending a message to the Wifi application to disconnect
@param req HTTP request for which the uri needs to be handled
@return ESP_OK 
*/
